...
let
  ...
  internalMonitor = lib.mkOption {
    ...
    description = ''
      ...
      output only; every other connected monitor is disabled while greeting, so
      the login box appears solely on the internal screen.
    '';
  };
  ...
  # on the internal panel. cage 0.3.0 can neither disable nor pick a named
